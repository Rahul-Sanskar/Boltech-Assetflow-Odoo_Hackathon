...
const Maintenance = () => {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      await API.post('/maintenance', {
        ...
        requestedById: Number(user.employeeId),
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Maintenance Requests" subtitle={`${requests.filter(r => r.status === 'Pending').length} pending requests`} actionLabel="New Request" onAction={() => setShowCreate(true)} />
      <DataTable columns={columns} data={requests} emptyMessage="No maintenance requests." />

      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="New Maintenance Request">
