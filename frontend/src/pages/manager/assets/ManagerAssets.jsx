...
const ManagerAssets = () => {
  ...
  const [form, setForm] = useState({
    ...
    location: '', departmentId: '', categoryId: '', acquisitionCost: '', isBookable: false
  });
  ...
  const columns = [
    ...
    { label: 'Location', key: 'location', render: (row) => row.location || '—' },
  ...
  return (
    <div>
      <PageHeader title="Department Assets" subtitle={`${assets.length} assets in your department`} actionLabel="Register Asset" onAction={() => setShowCreate(true)} />
      <DataTable columns={columns} data={assets} emptyMessage="No assets found in your department." />
      ...
      <Modal isOpen={showCreate} onClose={() => setShowCreate(false)} title="Register New Asset" size="lg">
        <form onSubmit={handleCreate} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="flex items-center gap-2">
            <input type="checkbox" id="isBookable" checked={form.isBookable} onChange={(e) => setForm({ ...form, isBookable: e.target.checked })} className="rounded" />
            <label htmlFor="isBookable" className="text-sm text-[var(--text-main)]">Shared / Bookable Resource</label>
          </div>
          <div className="sm:col-span-2 flex justify-end gap-3 pt-2">
            ...
            <button type="submit" className="px-5 py-2.5 rounded-xl bg-[var(--brand-primary)] hover:bg-[var(--brand-hover)] text-white text-sm font-semibold transition-all duration-200 hover:shadow-lg hover:shadow-[var(--brand-primary)]/25 active:scale-95">Register Asset</button>
          </div>
        </form>
      </Modal>
    </div>
